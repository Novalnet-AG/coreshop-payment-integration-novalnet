    ...
    public function execute($request)
    {
        ...
        if (!empty($requestParams['transition']) && $payment->getState() !== CorePaymentInterface::STATE_AUTHORIZED) {
            throw new ValidationException(
                sprintf("Unable to capture the Payment from the current state '%s'", $payment->getState())
            );
        }

        if (!empty($additionalData['NnTransactionTid']) && !empty($additionalData['NnTransactionStatus']) && $additionalData['NnTransactionStatus'] == 'ON_HOLD' && empty($additionalData['NnWebhookCapture'])) {

            $response = $this->api->doCapture((array) $additionalData);
            ...
            if (!empty($response['result']['status']) && $response['result']['status'] == 'SUCCESS') {
                ...
                $additionalData['CaptureApiProcessedAt'] = date('Y-m-d H:i:s');
                $message = sprintf($this->novalnetHelper->translate('novalnet.transaction.capture.label', $order->getLocaleCode()), $additionalData['CaptureApiProcessedAt']);
        ...
        } elseif (empty($additionalData['NnTransactionTid'])) {
