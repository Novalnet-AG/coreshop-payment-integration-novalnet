...
final class NovalnetInvoiceGuaranteeType extends AbstractType
{
    /**
     * {@inheritdoc}
     */
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        $builder
            ...
            ->add('min_order_amount', TextType::class, [
                'constraints' => [
                    new NotBlank([
                        'groups' => 'coreshop',
                    ]),
            ...
            ->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $event) {
                ...
                if (!empty($data['min_order_amount'])) {
                    if (!is_numeric($data['min_order_amount'])) {
                        $errMsg = ($locale == 'DE') ? 'Bitte geben Sie eine gültige Zahl in dieses Feld ein "Mindestbestellbetrag für Zahlungsgarantie"' :
                            'Please enter a valid number in this field "Minimum order amount for payment guarantee"';

                        throw new ValidationException($errMsg);
                    }

                    if (is_numeric($data['min_order_amount']) && $data['min_order_amount'] < 9.99) {
                        $errMsg = ($locale == 'DE') ? 'Bitte geben Sie eine gültige Zahl in dieses Feld ein "Mindestbestellbetrag für Zahlungsgarantie" Der Mindestbestellwert sollte größer oder gleich 9.99 € sein.' :
                            'Please enter a valid number in this field "Minimum order amount for payment guarantee" Minimum order amount should be greater than or equal to 9.99 €';
